...
app.post('/hook/github/push', function (req, res) {
  ...
  .then(function (idx) {
    ...
    const backwards = tags[idx].version;
    ...
    if (currBackwards === backwards) {
      ...
      const r = comp(compares[0].value, compares[idx + 1].value);
  ...
  function compare (i, ifReturn) {
    ...
    .then(function (data) {
      ...
      if (ifReturn) return i - 1;
      // start to compare
      const prev = compares[i - 1];
      const next = compares[i];
      const r = comp(prev.value, next.value);
      console.log('checking:', prev.version, next.version);
      if (r.deletion.length > 0 || r.modified.length > 0) {
        console.log('last backwards version is', tag.version);
        return compare(i + 1, true);
      } else {
        return compare(i + 1);
      }
    });
